...
The orchestrator combines queue, state, scheduler, and executor in one system—eliminating operational complexity while providing production-grade features.
...
<CardGroup cols={2}>
  ...
  <Card title="Workflow DAGs" icon="diagram-project">
    Directed Acyclic Graphs with fan-in/fan-out, step conditions, template variables, output transforms, and human approval gates.
  </Card>
...
The orchestrator runs in three modes:
...
The orchestrator fits these patterns:
...
<CardGroup cols={2}>
  <Card title="Quick Start" icon="bolt" href="/docs/quickstart">
    Get orchestrator running in minutes. Clone repository, start infrastructure with Docker Compose, and trigger your first job.
  </Card>
...
<Tabs>
  ...
  <Tab title="Workflows">
    Workflows orchestrate multiple jobs into DAGs. Steps can depend on outputs from parent steps, have conditional execution based on step status, and include human approval gates.
  </Tab>
...
Contributing to the orchestrator or running it locally:
