...
Strait uses a universal `strait.json` configuration file that all three SDKs and the CLI can read. This replaces the TypeScript-specific `strait.config.ts` file (now deprecated).
...
SDK client configuration. These values are read by all three SDKs.
