...
The strait uses PostgreSQL 18 as its primary data store. All primary keys are UUIDv7 stored as `TEXT`. Schema is managed by 49 migrations that run automatically on startup.
...
`000001_init.up.sql` through `000049_event_trigger_support.up.sql`.
