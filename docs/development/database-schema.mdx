...
The strait uses PostgreSQL 18 as its primary data store. All primary keys are UUIDv7 stored as `TEXT`. Schema is managed by 55 migrations that run automatically on startup.
...
Migration 000055 extends the existing `webhook_deliveries` table:

- Added `event_trigger_id TEXT` column (FK to event_triggers)
- Relaxed `run_id` and `job_id` NOT NULL constraints to support event trigger deliveries
...
`000001_init.up.sql` through `000055_webhook_delivery_queue.up.sql`.

### Event Trigger Migrations

| Migration | Description |
| :--- | :--- |
| `000049_event_trigger_support` | Core `event_triggers` table and indexes |
| `000050_event_trigger_improvements` | Additional columns and constraints |
| `000051_durable_sleep` | `trigger_type` and `sleep_duration` columns |
| `000052_event_trigger_prefix_index` | `text_pattern_ops` index for prefix queries |
| `000053_event_emit_key` | `event_emit_key` column for event chaining |
| `000054_event_trigger_sent_by` | `sent_by` audit column |
| `000055_webhook_delivery_queue` | Extend `webhook_deliveries` for event trigger webhooks |

### Adding New Migrations

1. Create a new pair of SQL files in the `migrations/` directory:
   - `000056_your_feature.up.sql`
   - `000056_your_feature.down.sql`
