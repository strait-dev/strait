...
The orchestrator uses PostgreSQL 18 as its primary data store. All primary keys are UUIDv7 stored as `TEXT`. Schema is managed by 48 migrations that run automatically on startup.
...
`000001_init.up.sql` through `000048_add_job_dependencies.up.sql`.

### Adding New Migrations

1. Create a new pair of SQL files in the `migrations/` directory:
   - `000049_your_feature.up.sql`
   - `000049_your_feature.down.sql`
