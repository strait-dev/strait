...
| :--- | :--- |
| `000049_event_trigger_support` | Complete event triggers schema — table, all columns, indexes, webhook delivery extensions. Squashed from 7 development migrations into one production-ready migration. |

<Note>
The event trigger schema was developed across 7 incremental migrations (000049–000055) during development. These were squashed into a single `000049_event_trigger_support` migration before the first production deploy to reduce startup time and rollback complexity.
</Note>
