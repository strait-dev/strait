...
- **Count**: Migrations currently run through the `000060_*` series and continue incrementally.
...
Set `FF_REDIS_REQUIRED=true` to enforce strict Redis availability. When enabled, Redis failures will propagate as errors instead of being silently absorbed.

| Variable | Default | Description |
| :--- | :--- | :--- |
| `FF_REDIS_REQUIRED` | `false` | When `true`, Redis failures are not absorbed. |
