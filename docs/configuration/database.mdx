...
The orchestrator relies on PostgreSQL for persistent storage and job queuing, and Redis for real-time pub/sub and CDC event streaming.
...
The orchestrator uses `pgx/v5` for high-performance connection pooling. You can tune the pool using the following variables:
...
The orchestrator implements an exponential backoff strategy for database connection failures:
