...
```bash
orchestrator trigger job_123 --payload '{"key": "value"}'
```

Trigger by slug with payload from file:
```bash
orchestrator trigger my-job-slug --project my-project --payload-file payload.json
```

Trigger with payload from stdin:
```bash
echo '{"key": "value"}' | orchestrator trigger job_123 --payload -
```
...
```bash
orchestrator jobs trigger job_123 --scheduled-at "2026-03-07T12:00:00Z"
```
...
```bash
orchestrator jobs trigger-bulk job_123 --items-file batch.json
```
