...
```bash
orchestrator backup create --output backup.sql --format custom
```
...
```bash
orchestrator backup restore --input backup.sql --clean --yes
```
...
<Tip>
You can provide an optional argument to specify the number of migrations to apply (e.g., `orchestrator migrate up 2`).
</Tip>
...
```bash
orchestrator migrate create <name>
```
...
```bash
orchestrator db shell --query "SELECT * FROM runs LIMIT 10;"
```
...
```bash
orchestrator cleanup --runs-older-than 30d --status completed --yes
```
