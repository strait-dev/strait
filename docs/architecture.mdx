...
The system uses PostgreSQL with the following primary tables (61 migrations):
...
The health check system uses a component registry (`internal/health/registry.go`) where each subsystem registers its health check function. This enables extensible readiness verification without modifying the health endpoint handler.
