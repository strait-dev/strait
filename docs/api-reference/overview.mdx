...
description: "Authentication, error formats, and common patterns for the Orchestrator API."
---

## Authentication

The Orchestrator API supports three authentication methods depending on the client and use case.
...
### API Keys
Used for project-specific operations. API keys are prefixed with `orc_`.
- **Header**: `Authorization: Bearer orc_...`
...
```bash
curl -X GET "https://api.orchestrator.com/v1/jobs?project_id=proj_1" \
     -H "Authorization: Bearer orc_live_abc123" \
