...
A `BatchOperation` tracks the outcome of a bulk trigger request against a [Job](/docs/concepts/jobs). When you trigger a job with multiple items in a single request (via `POST /v1/jobs/{jobID}/trigger-bulk`), Strait creates a batch operation to record how many items were submitted and how many [Runs](/docs/concepts/runs) were actually created.

## BatchOperation Model

The `BatchOperation` struct (defined in `internal/domain/types.go`) contains the following fields:
...
4. `created_count` reflects the number of runs that were actually created. This may be lower than `item_count` if some items were deduplicated (see [dedup_window_secs](/docs/concepts/jobs)).
...
- [Jobs](/docs/concepts/jobs): The job definition that was bulk-triggered.
- [Runs](/docs/concepts/runs): Individual execution instances created by the batch. Each run's `batch_id` links back to its originating batch operation.
