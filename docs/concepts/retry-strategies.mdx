...
description: "How the orchestrator handles execution failures."
---

When a job execution fails (non-2xx response or timeout), the orchestrator uses a retry strategy to determine when to attempt the execution again.

## Core Strategies

The system supports four primary retry strategies, defined in `internal/worker/backoff.go`.
...
When `FF_RUN_DLQ` is enabled, a run that exhausts its `max_attempts` will transition to the `dead_letter` state instead of `failed`. This allows engineers to inspect the failure and manually "replay" the run (resetting it to `queued`) once the underlying issue is resolved.
