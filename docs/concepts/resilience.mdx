...
The in-memory circuit breaker (`internal/worker/circuitbreaker.go`) provides fast lookups during dispatch. The PostgreSQL table provides durability.
...
When `attempt >= max_attempts` and the DLQ feature flag is enabled (`FF_RUN_DLQ=true`), the run transitions to `dead_letter` instead of `failed`. Dead-lettered runs can be replayed via the DLQ management API.
...
The stale reaper (`internal/scheduler/reaper.go`) detects runs that have stopped heartbeating and transitions them to a recoverable state:
