...
When the event arrives (via `POST /v1/events/send`), the step completes with the event payload as output, and downstream steps can access it via `{{parent_outputs.aml-check.*}}`.
