---
title: "Jobs"
description: "The fundamental unit of work in the strait."
---
...
Jobs are scoped to a `project_id`. The `slug` must be unique within that project. This allows for clean URLs and API access patterns like `/v1/projects/{project_id}/jobs/{slug}`.
...
<Tip>
Always use the `webhook_secret` to verify the `X-Webhook-Signature` header in your receiving service.
</Tip>

## Environment Binding

Jobs can be linked to an `environment_id`. This allows the strait to apply environment-specific overrides, such as changing the `endpoint_url` when running in a "Staging" vs "Production" environment.
