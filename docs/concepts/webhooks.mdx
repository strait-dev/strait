...
Event trigger webhooks (configured via `notify_url` on wait-for-event steps) use a **durable delivery** model. Deliveries are persisted to the `webhook_deliveries` table and processed by a background polling worker (`internal/webhook/event_notify.go`). This survives process restarts -- pending deliveries are picked up on the next poll cycle.
