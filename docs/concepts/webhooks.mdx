...
5. **Retry Logic**: If delivery fails, the orchestrator retries based on the response status code.
...
To ensure the webhook was sent by the orchestrator, every request includes an `X-Webhook-Signature` header if a `webhook_secret` is set for the job.

- **Header Format**: `sha256=<signature>`
- **Algorithm**: HMAC-SHA256 using the `webhook_secret` as the key and the raw JSON request body as the message.
...
The orchestrator attempts to deliver the webhook up to **3 times** with an exponential backoff:
- **Attempt 1**: Immediate
