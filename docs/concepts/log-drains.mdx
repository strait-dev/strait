...
The `LogDrain` struct (defined in `internal/domain/types.go`) contains the following fields:
...
- [Runs](/docs/concepts/runs): The execution instances whose log events are forwarded by log drains.
- [Jobs](/docs/concepts/jobs): The job definitions that produce the runs generating log events.
