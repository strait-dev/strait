...
When `FF_PROJECT_QUOTAS` is enabled and a project has a `MaxActiveEventTriggers` quota configured, the SDK `wait-for-event` endpoint enforces the limit:
...
Official SDK clients are available for TypeScript and Python in the `sdk/` directory:
...
const resolved = await events.sendEvent({
  eventKey: "approval:order-789",
...
Both SDKs support `waitForEvent`, `sendEvent`, `getEventTrigger`, and `cancelEventTrigger`.
