...
A `JobRun` represents a single execution instance of a [Job](/concepts/jobs). It tracks the state, payload, result, and timing of the execution.

## JobRun Model

The `JobRun` struct (defined in `internal/domain/types.go`) contains the following fields:
...
- **system_failed**: Internal orchestrator error (e.g., database failure).
...
When `FF_EXECUTION_TRACING` is enabled, the `execution_trace` field captures granular timing:
