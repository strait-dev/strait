...
Event Sources and [Event Triggers](/docs/concepts/event-triggers) serve different purposes:
...
The `EventSource` struct (defined in `internal/domain/types.go`) contains the following fields:
...
- [Event Triggers](/docs/concepts/event-triggers) -- Durable waits for events within workflow steps and runs.
- [Jobs](/docs/concepts/jobs) -- The unit of work that event subscriptions can target.
- [Workflows](/docs/concepts/workflows) -- Multi-step orchestrations that event subscriptions can target.
