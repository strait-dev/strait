---
title: "Deployment"
description: "Guide for deploying and scaling the orchestrator in production."
---

The orchestrator is designed as a single Go binary that can be deployed in various environments, from local development with Docker Compose to production on Fly.io or Kubernetes.

## Operation Modes

The orchestrator can run in three different modes, controlled by the `--mode` flag or the `MODE` environment variable:
...
The orchestrator requires the following backing services:
...
The orchestrator implements graceful shutdown for both the API and the worker:
