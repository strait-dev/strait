---
title: "Deployment"
description: "Guide for deploying and scaling the strait in production."
---

The strait is designed as a single Go binary that can be deployed in various environments, from local development with Docker Compose to production on Fly.io or Kubernetes.

## Operation Modes

The strait can run in three different modes, controlled by the `--mode` flag or the `MODE` environment variable:
...
The project includes a multi-stage Dockerfile at `apps/strait/Dockerfile` that produces a minimal Alpine-based image.

### Build the Image
```bash
docker build -t strait -f apps/strait/Dockerfile .
```
...
The strait requires the following backing services:
...
The repository includes `apps/strait/fly.toml` for quick deployment to Fly.io.
...
The strait implements graceful shutdown for both the API and the worker:
