...
<Note>
  Job groups are gated by the `FF_JOB_GROUPS` feature flag.
</Note>

### Create a Job Group
```bash
curl -X POST https://api.orchestrator.com/v1/job-groups \
...
curl -H "Authorization: Bearer $API_KEY" \
  https://api.orchestrator.com/v1/job-groups?project_id=proj_123
```

### Update a Job Group
```bash
curl -X PATCH https://api.orchestrator.com/v1/job-groups/{groupID} \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
### Delete a Job Group
```bash
curl -X DELETE https://api.orchestrator.com/v1/job-groups/{groupID} \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl -X POST https://api.orchestrator.com/v1/jobs \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "project_id": "proj_123",
...
```bash
curl -H "Authorization: Bearer $API_KEY" \
  https://api.orchestrator.com/v1/job-groups/{groupID}/jobs
```
