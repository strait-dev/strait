...
```bash
curl -X POST https://api.orchestrator.com/v1/jobs/{jobB_ID}/dependencies \
...
curl -H "Authorization: Bearer $API_KEY" \
  https://api.orchestrator.com/v1/jobs/{jobID}/dependencies
```

### Delete a Dependency
```bash
curl -X DELETE https://api.orchestrator.com/v1/jobs/{jobID}/dependencies/{dependencyID} \
  -H "Authorization: Bearer $API_KEY"
```
...
1. The orchestrator checks if all dependencies are met.
