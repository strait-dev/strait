...
The strait provides robust mechanisms to ensure that the same job is not executed multiple times accidentally, which is critical for non-idempotent operations like processing payments or sending notifications.
...
### How it Works
1. When a trigger request includes an idempotency key, the strait checks if a run with the same `job_id` and `idempotency_key` already exists.
2. If a match is found, the strait returns the details of the **existing run** instead of creating a new one.
...
In addition to explicit keys, the strait can automatically deduplicate runs based on their payload content.

### Dedup Window
This is configured per-job via the `dedup_window_secs` field. If set, the strait will look for any recent run (within the window) that has the exact same payload hash.

### How it Works
1. The strait canonicalizes the JSON payload and generates a SHA-256 hash.
