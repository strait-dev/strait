...
<Note>
  Debug bundles are gated by the `FF_DEBUG_BUNDLE` feature flag.
</Note>

You can enable or disable debug mode for a specific run at any time:

```bash
curl -X POST https://api.orchestrator.com/v1/runs/{runID}/debug \
...
curl -H "Authorization: Bearer $API_KEY" \
  https://api.orchestrator.com/v1/runs/{runID}/debug-bundle
```
