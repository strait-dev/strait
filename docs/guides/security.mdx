---
title: "Security"
description: "Security features and protections implemented in the strait."
---

The strait includes several built-in security measures to protect the infrastructure and ensure the integrity of job executions.
...
The strait implements rate limiting at two levels using the `httprate` middleware.
...
Users accessing the strait through the app are authorized via project roles. Each role defines a set of permissions identical to API key scopes. Four system roles (admin, operator, viewer, triggerer) are available, and custom roles can be created per-project.
...
<Note>
  Security is a shared responsibility. While the strait provides these protections, ensure your job endpoints also implement proper authentication and input validation.
</Note>
