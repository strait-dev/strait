...
Use the official TypeScript SDK for type-safe event trigger operations:
...
const trigger = await events.waitForEvent(runId, {
  eventKey: `review:${documentId}`,
  timeoutSecs: 172800, // 48 hours
  notifyUrl: "https://myapp.com/webhook/review-ready",
});

// In your review dashboard — approve the document
const resolved = await events.sendEvent({
  eventKey: `review:${documentId}`,
  payload: { approved: true, reviewer: "alice", comments: "Looks good" },
});

// Check trigger status
const status = await events.getEventTrigger(`review:${documentId}`);
...
Event triggers require `FF_EVENT_TRIGGERS=true` in your environment configuration. All event trigger API endpoints return 404 when the flag is disabled.
