...
Every API key has a set of scopes that restrict what it can do. Scopes are checked on every request by the `requirePermission` middleware. See [Authentication & Authorization](/guides/authentication) for the full scope list.
