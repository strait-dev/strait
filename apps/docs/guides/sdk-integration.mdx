...
<Info>
Managed agents use a separate runtime package, [`@strait/agents-sdk`](/docs/sdks/agents), for run-token callbacks, workflow state, and provider adapters.
</Info>
...
Managed agents in this repository do not use the public management client inside the executing runtime. They use `@strait/agents-sdk`, which is built specifically for `/sdk/v1/runs/{runID}/...` callbacks.
