...
- `X-Strait-Replay-Key`: stable across every retry of the same delivery row (derived deterministically from the delivery id). This is the key to use when deduping replays on the subscriber — if your handler sees the same replay key twice, it is safe to treat the second invocation as a no-op.
