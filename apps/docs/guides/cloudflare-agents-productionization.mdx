...
- `STR-294` outbound-worker sandboxing
...
6. Outbound egress from the runtime Worker is evaluated by the outbound Worker.
...
1. Set the Cloudflare env vars in Strait:
   ...
   - `CF_SANDBOX_MODE=outbound_worker`
   - `CF_OUTBOUND_WORKER_NAME`
...
1. build all bundles
2. deploy outbound Worker
3. deploy dispatch Worker
...
- Deploy the outbound Worker and dispatch Worker.
- Create a fresh agent with an outbound policy allowlist.
...
- check the outbound Worker response headers:
