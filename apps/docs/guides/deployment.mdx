...
Strait ships as a single Go binary. You can deploy it anywhere from Docker Compose locally to Fly.io or Kubernetes in production.
...
email verification and Polar billing in development. Doppler is optional and
intended only for internal Strait-managed environments.
...
# Internal-only Doppler dev path
cd apps/app && bun run dev:doppler
```
