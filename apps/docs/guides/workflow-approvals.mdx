...
Approval steps automatically create a parallel [Event Trigger](/concepts/event-triggers) when the workflow engine starts them. This means approvals can be resolved via either:

1. **The approve API endpoint**: `POST /v1/workflow-runs/{id}/steps/{ref}/approve`
