...
<CardGroup cols={2}>
  ...
  <Card title="Webhooks" icon="send">
    HMAC-SHA256 signed webhooks with automatic retries. Failed deliveries go to a review queue so nothing is silently lost.
  </Card>
...
<Steps>
  ...
  <Step title="Parallel Job Execution">
    Strait runs jobs concurrently with bounded backpressure to prevent memory exhaustion during traffic spikes. Panic recovery and graceful shutdown are built in.
  </Step>
  ...
  <Step title="Workflow Orchestration">
    Define steps that run in parallel or sequentially, with conditions, data passing, and human approval gates. Strait safely handles the case where multiple parent steps complete at the same time. Sub-workflows let you nest pipelines to any depth.
  </Step>
  <Step title="Observability First">
    OpenTelemetry tracing links job runs across API server, worker, and external endpoints. Prometheus metrics expose queue depth, throughput, and latency. Structured JSON logging supports log aggregation. Real-time streaming keeps your dashboards current.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Architecture" icon="sitemap" href="/architecture">
    Deep dive into internals. Learn about queue mechanics, run lifecycle, the workflow engine, and technology choices.
  </Card>
</CardGroup>
...
<Tabs>
  ...
  <Tab title="Runs">
    Runs represent a single execution of a job. Each run moves through a clear lifecycle — queued, dequeued, executing, completed, failed, timed out, or sent to the review queue — with events, logs, and usage data tracked along the way.
  </Tab>
  <Tab title="Workflows">
    Workflows orchestrate multiple jobs as dependency graphs. Steps can depend on outputs from parent steps, run conditionally based on step status, include human approval gates, and wait for external events.
  </Tab>
  <Tab title="Event Triggers">
    Event triggers pause workflow steps or job runs until an external event arrives via API. Waits are durable — stored in the database — and can last days or weeks. You can set timeouts, receive webhook notifications, chain events, and stream updates in real time.
  </Tab>
...
<CardGroup cols={2}>
  ...
  <Card title="Jobs vs Agents" icon="split" href="/guides/jobs-vs-agents">
    Learn when to use Jobs, when to use Agents, and when an Agent should trigger a heavier Job or Workflow.
  </Card>
...
- Understand the [workflow engine](/architecture#workflow-engine) and how dependency graphs execute
...
- Build durable workflows with [event triggers](/concepts/event-triggers) — wait for external events without holding resources
