...
Agents are not meant to replace Jobs.

They are the fast orchestration layer for model-driven execution. When the work becomes compute-heavy, long-running, or container-specific, the best pattern is for an agent to trigger a Strait Job or Workflow and stay focused on the interactive orchestration layer.

Jobs and Agents share one Strait account, but they are intentionally different product surfaces:

- Jobs are the durable compute product
- Agents are the low-latency orchestration product
- usage and pricing should be explained separately even when both surfaces live in the same dashboard
...
<CardGroup cols={2}>
  <Card title="Jobs vs Agents" icon="split" href="/docs/guides/jobs-vs-agents">
    Learn when to stay in the agent runtime and when to hand work off to Fly-backed Jobs or Workflows.
  </Card>
