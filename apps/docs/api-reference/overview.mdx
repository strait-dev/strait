...
|Agents|`/v1/agents`|Manage agent definitions, deployments, and runs|
...
The agent control-plane endpoints live under `/v1/agents`.

Use them to:

- create and update agents
- deploy versioned agent runtimes
- trigger agent runs
- list runs associated with an agent
...
See [Agent API Guide](/docs/guides/agent-api) for the expected lifecycle and curl examples.

## Runtime Callback Endpoints

Executing agents do not call the `/v1/agents` control-plane routes directly.

They use JWT run-token authenticated callback routes under `/sdk/v1/runs/{runID}/...`, typically through [`@strait/agents`](/docs/sdks/agents).

Important runtime callback endpoints include:
...
- `/v1/agents` is the management surface
- `/sdk/v1/runs/...` is the executing runtime surface
