...
description: "Conceptual overview of the Postgres schema — tables, relationships, state machines, CDC integration, and migration conventions."
icon: "database"
---
...
| `status` | `TEXT NOT NULL` | Current FSM state (see [Run State Machine](#run-state-machine)) |
...
Runs follow a 15-state finite state machine enforced in application code (`domain/fsm.go`). The FSM is the single source of truth for which transitions are legal — the database stores the current state as a plain `TEXT` column, and every state change passes through `domain.ValidateTransition`.
...
| `status` | `TEXT` | Workflow FSM state |
| `triggered_by` | `TEXT` | |

**Workflow Run FSM** (10 states):
...
| `status` | `TEXT` | Step FSM state |

**Step Run FSM** (7 states): `pending` -> `waiting` -> `running` -> `completed` | `failed` | `skipped` | `canceled`.
...
Transactional outbox for reliable run enqueue. When a caller needs to atomically commit a database transaction and enqueue a run, the intent is written to this table inside the same transaction. A background flusher promotes rows into `job_runs` using `FOR UPDATE SKIP LOCKED`.
...
<Tip>
  The outbox pattern eliminates the reliability gap between committing a user-facing transaction and calling `queue.Enqueue`. Multiple flushers are safe by construction via `FOR UPDATE SKIP LOCKED`.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Status as TEXT, not ENUM">
    Run status is stored as `TEXT` rather than a Postgres `ENUM`. This avoids migration pain when adding new states (Postgres enums cannot have values removed, and adding values requires careful transaction handling). The FSM is enforced in Go code with the `domain.ValidateTransition` function.
  </Accordion>
