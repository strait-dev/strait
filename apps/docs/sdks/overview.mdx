...
The monorepo also includes a separate runtime-side package, `@strait/agents-sdk`, for managed agent execution, callbacks, workflow state, and provider adapters.
...
`@strait/agents-sdk` is not the public management client. It is the runtime package used by managed agents inside an executing run.
