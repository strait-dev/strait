...
- Runtime DAG expansion via step outputs (`dynamic_steps`) without mutating workflow versions

## DAG runtime model

A workflow run materializes as many `workflow_step_runs` as there are steps in the selected workflow version snapshot.
When runtime expansion is used, additional per-run step definitions are stored separately and merged into runtime reads for that workflow run only.
...
A completed step can request per-run DAG expansion by returning:
...
The runtime validates the proposed steps before materializing them:
- duplicate `step_ref` values are rejected
- dependencies must point to an existing runtime step or another proposed dynamic step
- the combined static + dynamic graph must remain acyclic
- the total runtime step count must stay within the workflow step limit

Dependencies that were already satisfied before insertion are pre-counted, so newly added children can become runnable in the same callback pass when eligible.
