...
| `executing` | `completed`, `failed`, `timed_out`, `crashed`, `canceled`, `waiting`, `queued`, `dead_letter` |
...
- **dead_letter**: Moved to the review queue (a holding area where you can inspect and replay jobs that exhausted all retries).

## Terminal vs Non-Terminal States

A run is considered **terminal** when it reaches a state where no further automatic transitions occur (except for manual review queue replay).

<Info>
Terminal states: `completed`, `failed`, `timed_out`, `crashed`, `canceled`, `expired`.
</Info>
