...
An event source is a named inbound channel. External systems dispatch events to a source by name, and the source routes them to one or more jobs or workflows via **subscriptions**. Each subscription can optionally filter on the event payload, so a single source can fan out (deliver one event to multiple targets simultaneously).
...
A subscription binds an event source to a target (a job or workflow). When an event arrives at the source, each enabled subscription is evaluated and, if matched, triggers its target.
