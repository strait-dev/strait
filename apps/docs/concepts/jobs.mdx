...
| `poison_pill_threshold` | `*int` | If the same error repeats this many times in a row, Strait stops retrying and moves the run to the review queue (a holding area where you can inspect and replay jobs that exhausted all retries). `null` or `0` disables this. See [Resilience](/concepts/resilience#poison-pill-detection). |
