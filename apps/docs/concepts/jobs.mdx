...
A job defines a task you want Strait to execute — an HTTP endpoint to call, how often to call it, and what to do when things go wrong. Every time a job runs, Strait creates a [Run](/concepts/runs) to track that execution.

Jobs are the right execution model when the work needs durable container-backed execution on Fly Machines:
...
A job includes the following fields:
...
| `cron` | `string` | Cron expression for automatic scheduling (five fields: minute, hour, day-of-month, month, day-of-week). |
| `cron_overlap_policy` | `string` | Behavior when a cron tick fires with active runs: `allow` (default), `skip`, or `cancel_running`. See [Scheduling](/concepts/scheduling). |
...
| `poison_pill_threshold` | `*int` | If the same error repeats this many times in a row, Strait stops retrying and moves the run to the dead letter queue (a holding area for runs that need manual inspection). `null` or `0` disables this. See [Resilience](/concepts/resilience#poison-pill-detection). |
...
| `version_policy` | `string` | How queued runs interact with updates: `pin` (default), `latest`, or `minor`. See [Versioning](/concepts/versioning). |
...
- **Cron**: Standard five-field cron syntax (minute, hour, day-of-month, month, day-of-week).
...
- **Rate Limiting**: Prevents all runs from firing at the same time by capping the number of runs over a sliding window.
...
- [Workflows](/concepts/workflows): Dependency graphs (DAGs) that coordinate multiple jobs.
- [Jobs vs Agents](/docs/guides/jobs-vs-agents): How to choose between Fly-backed Jobs and Cloudflare-backed Agents.
