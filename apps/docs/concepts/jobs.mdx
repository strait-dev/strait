...
| `poison_pill_threshold` | `*int` | Consecutive identical errors before auto-quarantine to DLQ. `null` or `0` disables. See [Resilience](/concepts/resilience#poison-pill-detection). |
...
- [Workflows](/concepts/workflows): DAGs that coordinate multiple jobs.
