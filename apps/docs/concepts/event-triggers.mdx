...
description: "Pause execution until an external event arrives."
icon: "bolt"
---

Event triggers let your workflows and runs pause and wait for something to happen outside Strait — a webhook from a third-party service, a human approval, or a signal from another system. The wait can last minutes, hours, or weeks without consuming compute resources. When the event arrives, execution picks up exactly where it left off.
...
2. **Wait**: The step or run transitions to `waiting` status. No compute resources are consumed — the wait is tracked in the database.
...
| `sleep` | Durable sleep — waits until a specific time. Used by `sleep` workflow steps. |
...
Validation is enforced at all API entry points and during template rendering.
...
<Tip>
Set `timeout_secs` to a large value for long-running waits (e.g., `604800` for 7 days). The wait does not keep a worker busy.
</Tip>
...
Use standard duration strings: `30s` (30 seconds), `5m` (5 minutes), `1h` (1 hour), `24h` (1 day), `168h` (7 days).
...
One workflow's step completion can resume another workflow's waiting step.
...
When an event resolves a workflow step trigger, Strait wraps both updates — the trigger status and the step completion — in a single database transaction. If either fails, both roll back, preventing inconsistency.
...
### By Prefix (Resolve Multiple Triggers)

You can resolve all waiting triggers whose event key starts with a given prefix. This is useful when a batch operation completes and you want to unblock all related waiting steps at once:
...
- **4xx errors**: Moved to failed immediately (not retryable)
...
curl "https://strait.dev/v1/webhooks/deliveries?status=failed" \
  -H "Authorization: Bearer strait_..."

# Retry a failed delivery
curl -X POST https://strait.dev/v1/webhooks/deliveries/{deliveryID}/retry \
  -H "Authorization: Bearer strait_..."
```
...
The SSE stream delivers updates in sub-millisecond time.
...
The `?token=` parameter is accepted as an alternative to the `Authorization` header, specifically for browser clients that cannot set custom headers.
...
Terminal event triggers (`received`, `timed_out`, `canceled`) are automatically cleaned up based on the `EVENT_TRIGGER_RETENTION` setting.
