...
In multi-instance deployments, the reaper uses database-level locking so only one instance runs the reaper cycle. Other instances skip the cycle and try again on the next interval.
