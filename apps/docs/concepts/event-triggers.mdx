...
In multi-instance deployments, the reaper uses database-level locking to ensure exactly one instance runs the reaper cycle at a time. Other instances skip gracefully and try again on the next interval.
