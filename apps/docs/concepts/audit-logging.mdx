...
By default, every project retains audit events for the server-wide window (`AUDIT_RETENTION_DEFAULT_DAYS`, 365 days). Admins can override this per project through two endpoints:
...
- `PUT /v1/projects/{id}/audit/retention` — body `{days}`. `days > 0` overrides the default; `days = 0` disables retention trimming entirely for the project (audit events are never deleted by the reaper); `days < 0` is rejected with 400.

Both endpoints require admin access, are tenant-isolated to the authenticated project, and every successful `PUT` emits an `audit.retention_updated` event recording `old_days` and `new_days` for the audit trail.
