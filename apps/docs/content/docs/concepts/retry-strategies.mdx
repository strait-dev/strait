...
By default, retried runs re-enter the queue at their original priority. In a deep queue, this means retries wait behind new runs, increasing end-to-end latency for failing jobs.

The `retry_priority_boost` field increases a run's queue priority on each retry, so retried runs are dequeued sooner.

| Value | Behavior |
| :--- | :--- |
| `1` (default) | Each retry increases priority by 1. A run starting at priority 0 reaches priority 3 after 3 retries. |
| `0` | No boost. Retries re-enter at their original priority. |
| `2`-`10` | Aggressive boost for latency-sensitive jobs. A boost of 2 reaches priority 6 after 3 retries. |

Priority is always capped at 10 (the maximum queue priority). The boost applies to both failure retries and timeout retries.
...
```json
{
  ...
  "max_attempts": 5,
  "retry_priority_boost": 2
}
```

With this configuration, a run starting at priority 0 would be retried at priorities 2, 4, 6, 8 across its 4 retry attempts. To disable boost after job creation, update the job with `retry_priority_boost: 0` via PATCH.

## Configuration

Retries can be configured at multiple levels:

- **Job Level**: Set `retry_strategy`, `retry_delays_secs`, and `retry_priority_boost` on the Job definition.
