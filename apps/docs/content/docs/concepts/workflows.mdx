...
| `wait_for_event` | Pauses until an external event arrives via API. See [Event Triggers](/docs/concepts/event-triggers). |
...
Pauses the workflow until an external event arrives via API. The step creates an [Event Trigger](/docs/concepts/event-triggers) with a globally unique `event_key` and optional timeout. No goroutines are held — the wait is a database row.
