...
| `retry_priority_boost` | `int` | Priority increase per retry attempt (0-10, default: 1). Set to 0 to disable. See [Retry Strategies](/docs/concepts/retry-strategies#priority-boost). |
| `poison_pill_threshold` | `*int` | Consecutive identical errors before auto-quarantine to DLQ. `null` or `0` disables. See [Resilience](/docs/concepts/resilience#poison-pill-detection). |
...
<Tabs items={["Cron Scheduling","Execution Window"]}>
  <Tab value="Cron Scheduling">
    ```json
{
  "cron": "0 * * * *",
  "timezone": "UTC"
