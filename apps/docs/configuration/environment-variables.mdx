...
| `TERMINAL_ARCHIVE_ENABLED` | Boolean | `true` | Move terminal job runs to an archive table to keep the hot `job_runs` table lean. Disable only for testing. |
| `PARTITION_RECLAIM_ENABLED` | Boolean | `false` | Reclaim disk space by re-creating detached time-series partitions. Enable on long-running self-hosted installs with disk pressure. |
| `PARTITION_RECLAIM_INTERVAL` | Duration | `24h` | How often the partition reclaim sweep runs. |
| `PARTITION_RECLAIM_SAFETY_MONTHS` | Integer | `2` | Minimum months of data to retain when reclaiming partitions. |
...
| `SSE_MAX_CONNS` | Integer | `5000` | Maximum total concurrent SSE connections across all projects. |
| `SSE_MAX_CONNS_PER_PROJECT` | Integer | `100` | Maximum concurrent SSE connections per project. |
| `SSE_MAX_CONN_DURATION` | Duration | `30m` | Maximum duration for a single SSE connection before the server closes it. |
...
Strait is orchestration-only. Workers connect via gRPC streaming and execute jobs in their own processes. There is no managed container runtime.

| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `GRPC_ENABLED` | Boolean | `true` | Enable the gRPC streaming server for worker connections. |
| `GRPC_PORT` | Integer | `50051` | Port the gRPC server listens on. |
| `GRPC_TLS_CERT_PATH` | String | - | Path to TLS certificate file. Optional; cloud deployments terminate TLS at the load balancer. |
| `GRPC_TLS_KEY_PATH` | String | - | Path to TLS private key file. |
| `GRPC_KEEPALIVE_TIME` | Duration | `30s` | How often the server sends keepalive pings to connected workers. |
| `GRPC_KEEPALIVE_TIMEOUT` | Duration | `10s` | Time the server waits for a keepalive ping acknowledgement before closing the connection. |
| `WORKER_HEARTBEAT_TIMEOUT` | Duration | `30s` | Duration after which a worker is considered disconnected if no heartbeat is received. |
| `WORKER_DB_SYNC_INTERVAL` | Duration | `10s` | Interval at which connected worker state is flushed to the database. |
| `WORKER_DISCONNECT_SWEEP_INTERVAL` | Duration | `30s` | Interval at which the scheduler sweeps for and cleans up stale worker records. |
| `EXTERNAL_API_URL` | String | - | Base URL workers use to call back to the Strait API (e.g., `https://api.example.com`). |
...
| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `DB_HEALTH_CHECK_PERIOD` | Duration | `30s` | How often to check database connectivity. |
| `DB_STATEMENT_TIMEOUT` | Duration | `30s` | Maximum time for a single SQL statement. |
| `DB_LOCK_TIMEOUT` | Duration | `10s` | Maximum time to wait for a database lock. |

## Worker Tuning

| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `MAX_DEQUEUE_BATCH_SIZE` | Integer | `10` | Number of runs to claim per dequeue cycle. |
| `DEFAULT_JOB_MAX_CONCURRENCY` | Integer | `0` (unlimited) | Default max concurrency when not set on the job. |
| `DEFAULT_RUN_TTL_SECS` | Integer | `0` (no expiry) | Default TTL for runs when not set on the job. |
...
| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `RESEND_API_KEY` | String | - | API key for email notifications via Resend. |
| `RESEND_FROM_EMAIL` | String | `noreply@strait.dev` | From address for email notifications. |

## Webhooks

| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `WEBHOOK_REQUIRE_TLS` | Boolean | `false` | Require HTTPS for all webhook endpoints. |
| `ALLOW_PRIVATE_ENDPOINTS` | Boolean | `false` | Allow webhook delivery to private IP ranges (not recommended for production). |

## Audit Events

| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `AUDIT_RETENTION_DEFAULT_DAYS` | Integer | `365` | Default retention in days for audit events. Per-project overrides live in `project_quotas.audit_retention_days`. `0` disables trimming for a project. |
| `AUDIT_ASYNC_BUFFER_SIZE` | Integer | `4096` | Size of the in-memory async emit buffer. Must be at least `256`. |
| `AUDIT_SIEM_ENDPOINT` | String | - | SIEM forwarding endpoint. Leave empty to disable. When set, `AUDIT_SIEM_AUTH_TOKEN` is required. |
| `AUDIT_SIEM_AUTH_TOKEN` | String | - | Bearer token sent to the SIEM endpoint as `Authorization: Bearer ...`. |
| `AUDIT_SIEM_BATCH_SIZE` | Integer | `100` | Number of audit events to batch per SIEM flush. |
| `AUDIT_SIEM_FLUSH_INTERVAL` | Duration | `10s` | Maximum time between SIEM flushes. |
| `AUDIT_EXPORT_ROW_CAP_DEFAULT` | Integer | `1000000` | Default cap on rows returned per audit export call. Per-project overrides live in `project_quotas.audit_export_row_cap`. |
| `AUDIT_DLQ_RECLAIM_BATCH` | Integer | `200` | Maximum deadletter rows the reclaimer processes per scheduler tick. |
| `AUDIT_DLQ_MAX_AGE_DAYS` | Integer | `30` | Maximum days a deadletter row may live before the retention reaper drops it. `0` disables the sweep. |
| `AUDIT_DLQ_MAX_RECLAIM_ATTEMPTS` | Integer | `10` | Maximum reclaim attempts per DLQ row before skipping. `0` disables the cap. |

## Code Deployments

| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `BUILDKIT_ADDRESS` | String | `tcp://buildkitd.strait-build.svc.cluster.local:1234` | BuildKit daemon address for building container images. |
| `BUILDKIT_ADDRESSES` | String | - | Comma-separated list of BuildKit daemon addresses for multi-node round-robin dispatch. Overrides `BUILDKIT_ADDRESS` when non-empty. |
| `BUILDKIT_NAMESPACE` | String | `strait-build` | Kubernetes namespace where BuildKit runs. |
| `BUILDKIT_CACHE_ENABLED` | Boolean | `true` | Enable BuildKit layer caching to speed up repeated builds. |
| `BUILD_MAX_TARBALL_MB` | Integer | `256` | Maximum size in MB of uploaded source tarballs. |
| `BUILD_TIMEOUT` | Duration | `10m` | Maximum time to wait for a container image build to complete. |
| `DEPLOYMENT_GC_ENABLED` | Boolean | `true` | Enable automatic garbage collection of stale pending and failed deployments. |
| `DEPLOYMENT_GC_INTERVAL` | Duration | `1h` | How often the deployment GC runs. |
| `DEPLOYMENT_GC_PENDING_TTL` | Duration | `15m` | Age at which a pending deployment is considered stale and removed. |
| `DEPLOYMENT_GC_FAILED_AGE` | Duration | `168h` | Age at which failed deployments are removed (default: 7 days). |
| `OBJECT_STORE_TYPE` | String | `s3` | Object store type. `s3` works for AWS S3, Cloudflare R2, and MinIO. |
| `OBJECT_STORE_BUCKET` | String | - | Bucket name for storing code bundles. |
| `OBJECT_STORE_ENDPOINT` | String | - | S3-compatible endpoint URL. Leave empty for AWS S3. |
| `OBJECT_STORE_REGION` | String | `auto` | Storage region. |
| `OBJECT_STORE_ACCESS_KEY` | String | - | Access key for object store authentication. |
| `OBJECT_STORE_SECRET_KEY` | String | - | Secret key for object store authentication. |
| `OBJECT_STORE_FORCE_PATH_STYLE` | Boolean | `false` | Use path-style S3 URLs. Set `true` for MinIO. |
