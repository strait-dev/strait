...
| `NOTIFY_SUBSCRIBER_TOKEN_ISSUER` | String | `strait-notify` | Issuer claim required for notify subscriber JWTs. |
| `NOTIFY_SUBSCRIBER_TOKEN_AUDIENCE` | String | `strait-notify-subscriber` | Audience claim required for notify subscriber JWTs. |
| `NOTIFY_EMAIL_PROVIDER` | String | `ses` | Notify email sender provider. Supported value: `ses`. |
| `NOTIFY_EMAIL_NORMALIZE_ENABLED` | Boolean | `true` | Normalize subscriber email addresses at upsert/update time. |
| `NOTIFY_EMAIL_VERIFY_ENABLED` | Boolean | `true` | Enable email verification checks on subscriber upsert/update. |
| `NOTIFY_EMAIL_VERIFY_MX` | Boolean | `true` | Require MX records during notify email verification when enabled. |
| `SES_REGION` | String | `us-east-1` | AWS SES region used for notify email delivery when provider is `ses`. |
| `SES_FROM_EMAIL` | String | `noreply@strait.dev` | Default `From` address for notify SES sends. |
| `SES_CONFIGURATION_SET` | String | - | Optional SES configuration set attached to notify sends. |
| `SES_ACCESS_KEY_ID` | String | - | Optional static AWS access key for SES. Prefer IAM role credentials. |
| `SES_SECRET_ACCESS_KEY` | String | - | Optional static AWS secret key for SES. |
| `SES_SESSION_TOKEN` | String | - | Optional AWS session token for SES static credentials. |
| `NOTIFY_SES_FEEDBACK_SQS_URL` | String | - | SQS queue URL used to ingest SES feedback events (SNS-to-SQS). |
| `NOTIFY_SES_FEEDBACK_POLL_INTERVAL` | Duration | `5s` | Sleep interval between empty SES feedback polls. |
| `NOTIFY_SES_FEEDBACK_WAIT_TIME_SECONDS` | Integer | `10` | SQS long-poll wait time for SES feedback receive calls (0-20). |
| `NOTIFY_SES_FEEDBACK_MAX_MESSAGES` | Integer | `10` | Maximum SQS feedback messages read per receive call (1-10). |
| `NOTIFY_SES_FEEDBACK_VISIBILITY_TIMEOUT_SECONDS` | Integer | `120` | SQS visibility timeout for SES feedback messages while processing. |
| `NOTIFY_CLEANUP_INTERVAL` | Duration | `30m` | Interval for notify retention cleanup (expired callback receipts, old suppression events). |
| `NOTIFY_CLEANUP_BATCH_SIZE` | Integer | `1000` | Maximum rows deleted per cleanup batch for notify retention loops. |
| `NOTIFY_SUPPRESSION_EVENT_RETENTION` | Duration | `720h` | Retention window for notify suppression events before cleanup removes old rows. |
...
| `NOTIFY_DELIVERY_MAX_ATTEMPTS` | Integer | `5` | Maximum attempts for scheduled notify message delivery before terminal failure. |
| `NOTIFY_RETRY_BASE_DELAY` | Duration | `30s` | Base delay used for exponential notify delivery retries. |
| `NOTIFY_RETRY_MAX_DELAY` | Duration | `15m` | Maximum delay cap for notify delivery retries. |
| `NOTIFY_DIGEST_MAX_ITEMS` | Integer | `50` | Maximum digest entries rendered into one digest message payload. |
| `NOTIFY_DIGEST_MAX_TITLE_CHARS` | Integer | `120` | Maximum title characters per digest line item before truncation. |
| `NOTIFY_ESCALATION_TIERS` | Integer | `3` | Number of escalation tiers emitted for pending approval reminders. |
| `NOTIFY_ESCALATION_MIN_INTERVAL` | Duration | `10m` | Minimum interval between approval escalation reminders. |
...
| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `COMPUTE_RUNTIME` | String | `none` | Container runtime for managed execution: `none`, `docker`, or `k8s`. |
| `K8S_NAMESPACE` | String | `default` | Kubernetes namespace for managed containers. Required when `COMPUTE_RUNTIME=k8s`. |
...
| `WARM_POOL_ENABLED` | Boolean | `false` | Enable warm container pool for faster cold starts. |
| `WARM_POOL_MAX_PER_JOB` | Integer | `3` | Maximum warm containers per job. |
| `WARM_POOL_TTL` | Duration | `5m` | TTL for idle warm containers before they are destroyed. |
...
| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `SSE_MAX_CONNS` | Integer | `1000` | Maximum total SSE connections. |
| `SSE_MAX_CONNS_PER_PROJECT` | Integer | `100` | Maximum SSE connections per project. |
| `SSE_MAX_CONN_DURATION` | Duration | `30m` | Maximum duration for a single SSE connection. |
...
| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `BUILDKIT_ADDRESS` | String | - | BuildKit daemon address for building container images. |
| `OBJECT_STORE_TYPE` | String | - | Object store type (`s3`). |
| `OBJECT_STORE_BUCKET` | String | - | S3 bucket for code bundles. |
| `OBJECT_STORE_REGION` | String | - | S3 region. |
| `CONTAINER_REGISTRY_TYPE` | String | - | Registry type (`ecr` or `generic`). |
| `CONTAINER_REGISTRY_URL` | String | - | Registry URL for pushing built images. |
