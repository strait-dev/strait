...
## Cloudflare Agents Runtime

These variables enable the Cloudflare-backed agent runtime and deploy path. When none of them are set, Strait continues to use the local agent stub/runtime path.

| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `CF_ACCOUNT_ID` | String | - | Cloudflare account identifier used for Workers for Platforms API calls. Required when Cloudflare agents are enabled. |
| `CF_API_TOKEN` | String | - | API token used to deploy and manage user Workers in the dispatch namespace. Required when Cloudflare agents are enabled. |
| `CF_DISPATCH_NAMESPACE` | String | - | Primary dispatch namespace for agent Workers in the current environment. Required when Cloudflare agents are enabled. |
| `CF_DISPATCH_NAMESPACE_STAGING` | String | - | Optional secondary namespace for staging or shadow deploys. |
| `CF_DISPATCH_WORKER_URL` | String | - | Public URL of the dynamic dispatch Worker that Strait calls to start remote agent runs. Must be a valid HTTP(S) URL. |
| `CF_OUTBOUND_WORKER_NAME` | String | - | Name of the Outbound Worker used for compatibility/fallback egress control when `CF_SANDBOX_MODE=outbound_worker`. |
| `CF_COMPATIBILITY_DATE` | String | - | Compatibility date applied to Cloudflare agent Workers and dispatch entrypoints. Required when Cloudflare agents are enabled. |
| `CF_SANDBOX_MODE` | String | `dynamic_worker` | Agent sandbox mode: `dynamic_worker`, `disabled`, or `outbound_worker`. `dynamic_worker` is the default. `outbound_worker` requires `CF_OUTBOUND_WORKER_NAME`. |

When running the Cloudflare deploy path, build the workers first:

```bash
cd apps/agents && bun run build
```

The Go server will deploy the built runtime bundle from `apps/agents/dist/runtime/worker.js` when present, and it will fall back to the embedded runtime snapshot checked into `apps/strait/internal/agents/runtime_worker_bundle.js` if the built artifact is absent.
