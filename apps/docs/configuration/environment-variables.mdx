...
<Tip>
  All environment variables are managed via [Doppler](https://doppler.com) (project: `strait`, configs: `dev` / `stg` / `prd`). Use `doppler run -- <cmd>` to inject secrets locally, or see the [Deployment Guide](/guides/deployment) for production setup.
</Tip>
