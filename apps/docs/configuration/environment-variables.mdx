...
| `QUEUE_ENGINE` | String | `legacy` | Queue storage engine: `legacy` uses `job_runs` as the claim table; `batchlog` claims from narrow queue log entries and keeps `job_runs` as the ledger. |
...
| `QUEUE_USE_DENORMALIZED_DEQUEUE` | Boolean | `false` | Use the fully denormalized dequeue path for the legacy engine. |
