...
| `CF_OUTBOUND_WORKER_NAME` | String | - | Name of the Outbound Worker used for egress control when `CF_SANDBOX_MODE=outbound_worker`. |
...
| `CF_SANDBOX_MODE` | String | `disabled` | Agent sandbox mode: `disabled` or `outbound_worker`. `outbound_worker` requires `CF_OUTBOUND_WORKER_NAME`. |
