...
| `AUDIT_DLQ_RECLAIM_BATCH` | Integer | `200` | Maximum audit dead-letter rows the reclaimer processes per scheduler tick. |
| `AUDIT_DLQ_MAX_AGE_DAYS` | Integer | `30` | Maximum days an audit dead-letter row may live before the retention reaper drops it. `0` disables the sweep. |
