...
description: "Customer-facing environment variables used to configure Strait."
sidebarTitle: "Environment Variables"
---

This page documents the environment variables most deployments need to configure Strait. Internal maintenance settings are intentionally omitted from the public docs.
...
| `REDIS_URL` | String | - | Redis connection string for pub/sub and real-time updates. |
...
| `WORKER_QUEUE_SIZE` | Integer | `0` | Bounded task queue size for each worker. When the queue is full, new work waits until capacity opens. `0` means unbounded. |
...
| `REAPER_INTERVAL` | Duration | `30s` | How often Strait checks for interrupted or stale runs. |
| `STALE_THRESHOLD` | Duration | `60s` | Threshold after which a run is considered stale. |
| `POLLER_INTERVAL` | Duration | `5s` | How often workers check for queued work. |
...
| `REAPER_DELETE_BATCH_SIZE` | Integer | `100` | Maximum records cleaned up in one retention pass. |
...
| `EVENT_TRIGGER_RETENTION` | Duration | `168h` (7d) | Retention period for terminal event triggers. Set to `0` to disable automatic cleanup. |
...
| `PARTITION_RECLAIM_INTERVAL` | Duration | `24h` | How often Strait checks whether old partitions can be reclaimed. |
...
| `WF_STALL_THRESHOLD` | Duration | `15m` | Max time without workflow progression before Strait considers the run stalled. |
...
Strait is orchestration-only. Workers connect via gRPC streaming and execute jobs in their own processes. There is no built-in container runtime.

| Variable | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `GRPC_ENABLED` | Boolean | `true` | Enable the gRPC streaming server for worker connections. |
| `GRPC_BIND_ADDR` | String | `127.0.0.1` | Address the gRPC server binds to. Non-loopback plaintext listeners require TLS or explicit `GRPC_ALLOW_PLAINTEXT=true`. |
| `GRPC_PORT` | Integer | `50051` | Port the gRPC server listens on. |
| `GRPC_ALLOW_PLAINTEXT` | Boolean | `false` | Permit plaintext gRPC on non-loopback bind addresses. Use only behind a trusted TLS-terminating proxy. |
...
| `WORKER_DISCONNECT_SWEEP_INTERVAL` | Duration | `30s` | How often Strait checks for disconnected workers. |
...
| `AUDIT_DLQ_RECLAIM_BATCH` | Integer | `200` | Maximum audit dead-letter rows processed in one recovery pass. |
| `AUDIT_DLQ_MAX_AGE_DAYS` | Integer | `30` | Maximum days an audit dead-letter row may be retained. `0` disables automatic cleanup. |
| `AUDIT_DLQ_MAX_RECLAIM_ATTEMPTS` | Integer | `10` | Maximum recovery attempts per audit dead-letter row before skipping. `0` disables the cap. |
