...
| `NOTIFY_EMAIL_PROVIDER` | String | `ses` | Notify email sender provider. `ses` is the standard mode. `resend` requires `NOTIFY_EMAIL_ALLOW_LEGACY_RESEND=true` and is intended for temporary rollback only. |
| `NOTIFY_EMAIL_ALLOW_LEGACY_RESEND` | Boolean | `false` | Allows legacy notify email sends through Resend when `NOTIFY_EMAIL_PROVIDER=resend` or when a provider config resolves to Resend. |
