...
| `SENTRY_TRACES_SAMPLE_RATE` | Float | `0.1` | Sentry trace sample rate from `0.0` to `1.0`. High-volume stream transactions are additionally filtered to a deterministic 1% sample before send. |
| `SENTRY_RELEASE` | String | - | Release identifier attached to Sentry events. Defaults to the binary version when unset. Use the same value for frontend sourcemap uploads. |
