...
<Callout type="info">
  Production and staging secrets are managed via [Doppler](https://doppler.com) (project: `strait`, configs: `dev` / `stg` / `prd`). For local development, prefer the built-in local defaults described in the [Deployment Guide](/docs/guides/deployment); Doppler is optional and intended for Strait-managed environments.
</Callout>
