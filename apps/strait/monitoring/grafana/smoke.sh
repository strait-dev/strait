...
STAGED_DASHBOARDS="$(mktemp -d)"
for dashboard in "$ROOT"/*.json; do
  jq '.dashboard // .' "$dashboard" >"${STAGED_DASHBOARDS}/$(basename "$dashboard")"
done
...
docker run --rm -d \
  ...
  -v "$STAGED_DASHBOARDS:/var/lib/grafana/dashboards/strait:ro" \
