CREATE TABLE IF NOT EXISTS job_run_ready_events (
    ...
    UNIQUE (run_id, ready_generation, reason)
);

CREATE INDEX IF NOT EXISTS idx_job_run_ready_events_latest
    ON job_run_ready_events(run_id, ready_generation, id DESC);
...
SELECT
    ...
    CASE
        ...
        ELSE GREATEST(s.updated_at, COALESCE(c.started_at, s.updated_at), COALESCE(ready.created_at, s.updated_at))
    END AS updated_at
