...
SELECT
    ...
    CASE WHEN t.run_id IS NULL THEN c.lease_owner ELSE NULL::TEXT END AS lease_owner,
    CASE WHEN t.run_id IS NULL THEN c.lease_expires_at ELSE NULL::TIMESTAMPTZ END AS lease_expires_at,
...
-- safety-ok: claimed_at duplicated started_at on the append-only active-claim
-- side table. The read view above now derives updated_at from started_at, and
-- application writes stop referencing claimed_at in the same release.
