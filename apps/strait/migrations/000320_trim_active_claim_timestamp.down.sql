...
UPDATE job_run_active_claims
SET claimed_at = COALESCE(claimed_at, started_at, created_at, NOW())
...
SELECT
    ...
    CASE WHEN t.run_id IS NULL THEN GREATEST(s.updated_at, COALESCE(c.claimed_at, s.updated_at)) ELSE t.updated_at END AS updated_at
