...
| `./internal/store` | 432 | 2125 | Store package has many DB accessors with little direct self-coverage; API-key persistence has no lived mutants with 37 killed and 27 not covered after covering create, revoke, rotation guard, not-found mapping, and row scanning branches; workflow snapshots have full dry coverage and no lived mutants with 11 killed after covering lookup, insert, scoped fetch, list scan, and row error paths. Worker recovery should stay on smaller targeted integration slices: a full integration-tagged workers dry probe did not return quickly enough for the local mutation loop. |
