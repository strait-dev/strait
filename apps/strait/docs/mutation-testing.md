...
| `./internal/scheduler` | 1042 | 196 | Broad scheduler surface; recovery/shutdown helpers, component registration, quota resume advisory-lock, backpressure token cap, counter reconciler transaction, cron, usage report and forecast emailers, usage flusher, batch flusher, contract expiry, webhook message cleanup, watched query, and stats aggregator slices are clean with 353 killed mutants. Continue splitting by reapers, monitors, and reconciliation helpers before a full run. |
