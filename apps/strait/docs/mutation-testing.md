...
| `./internal/api/grpc` | 348 | 178 | Broad worker-plane surface; auth, auth cache, replica ID, registry, recovery/logging interceptors, Sentry interceptor, worker metrics, server startup/TLS, dispatch helpers, and stream control-loop helper slices are covered with 338 killed mutants and one intentional forced-stop timeout gap. Remaining dispatch/stream/sweep gaps are DB-backed worker-task, audit, and end-to-end stream paths. |
...
| `./internal/worker` | 997 | 0 | Baseline package tests take about two minutes, so a full one-worker mutation run needs separate scheduling. Executor partition cycle, run timing, environment override, dispatch policy, dispatch trace, event channel, transitions, dispatch headers, completion, signals, dequeue, dispatch guards, dispatch job, dispatch readiness, retry, worker mode, worker requeue, billing, cache, timeout, system failure, failure transition, dispatch, dispatch HTTP, errors, handlers, HTTP client, config, lifecycle, drain, poll, event pipeline, observability, trace, job cache, job version cache, cache consistency, metadata cache, workflow cache, snooze, worker result, adaptive poll interval, adaptive controller, backoff, bulkhead, circuit breaker, database retry, health score, dispatch cache, DLQ cap, endpoint guard cache, heartbeat, heartbeat store, tracing middleware, pool, job clone, downstream completion, completion payload mapping, completion trigger, signing, ClickHouse subscriber, ClickHouse records, pubsub subscriber, validation, webhook, webhook delivery, and webhook signature slices are clean with 944 killed mutants. Continue bounded executor-path slices before a full run. |
