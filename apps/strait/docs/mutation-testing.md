...
| `./internal/api/grpc` | 282 | 244 | Broad worker-plane surface; auth cache, replica ID, and worker metrics helper slices are clean with 14 killed mutants. Continue splitting by auth, registry, dispatch, stream, and server helpers before a full run. |
