...
| `./internal/queue` | 763 | 19 | Broad queue surface; enqueue terminal error helper, backpressure, enqueue retry, PgQue route selection, PgQue claim helper, PgQue dequeue, PgQue ready, PgQue enqueue, PgQue runtime, health sampler, and notify slices are clean with 568 killed mutants. Continue splitting by run writer and metrics helpers before a full run. |
