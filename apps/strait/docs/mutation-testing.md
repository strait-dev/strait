...
| `./internal/scheduler` | 1025 | 213 | Broad scheduler surface; quota resume advisory-lock, backpressure token cap, counter reconciler transaction, cron, batch flusher, contract expiry, webhook message cleanup, watched query, and stats aggregator slices are clean with 226 killed mutants. Continue splitting by reapers, monitors, usage emailers, and reconciliation helpers before a full run. |
