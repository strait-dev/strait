...
| `./internal/scheduler` | 1044 | 194 | Broad scheduler surface; usage report candidate, backpressure sampler, anomaly monitor, core scheduler reloader, SLO webhook adapter, stale subscription checker, advisory lock, recovery/shutdown helpers, component registration, quota resume advisory-lock, backpressure token cap, counter reconciler transaction, cron, usage report and forecast emailers, usage flusher, batch flusher, contract expiry, webhook message cleanup, watched query, and stats aggregator slices are clean with 432 killed mutants. Continue splitting by reapers, monitors, and reconciliation helpers before a full run. |
