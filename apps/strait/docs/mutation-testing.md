...
| `./internal/queue` | 685 | 92 | Broad queue surface; enqueue terminal error helper, backpressure, enqueue retry, PgQue route selection, PgQue claim helper, PgQue dequeue, and PgQue ready slices are clean with 432 killed mutants. Continue splitting by PgQue enqueue, runtime, health sampler, notify, run writer, and metrics helpers before a full run. |
