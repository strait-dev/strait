...
| `./internal/queue` | 711 | 66 | Broad queue surface; enqueue terminal error helper, backpressure, enqueue retry, PgQue route selection, PgQue claim helper, PgQue dequeue, PgQue ready, and PgQue enqueue slices are clean with 457 killed mutants. Continue splitting by PgQue runtime, health sampler, notify, run writer, and metrics helpers before a full run. |
