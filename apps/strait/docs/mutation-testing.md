...
| `./internal/api/grpc` | 393 | 133 | Broad worker-plane surface; auth, auth cache, replica ID, registry, recovery/logging interceptors, Sentry interceptor, worker metrics, server startup/TLS, dispatch helpers, sweep, DB sync, and stream control-loop helper slices are covered with 435 killed mutants and one intentional forced-stop timeout gap. Sweep has no lived mutants with 30 killed and 2 constant-only not covered after covering stale-task recovery, stale-worker eviction, offline deletion, durable result handoff, finalizer retry, and recovered-run requeue paths; DB sync has no lived mutants with 5 killed and 1 constant-only not covered after covering queue selection and register error continuation; dispatch has full dry coverage and no lived mutants with 62 killed after covering worker-task record errors, duplicate assignment protection, abort cleanup, result handoff failures, audit emission, and default attempt normalization. Remaining stream gaps are mostly DB-backed worker-task and end-to-end stream paths. |
