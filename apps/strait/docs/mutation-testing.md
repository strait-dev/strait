...
| `./internal/testutil` | 17 | 249 | Test helper package has little direct self-coverage; existing testcrypto and pgxslow runnable helper slices are clean with 15 killed mutants, and the seed-pentest validation slice has no lived mutants with 3 killed and 16 not covered. Remaining gaps are mostly DB, Redis, factory, and assertion helper setup paths. |
