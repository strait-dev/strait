...
| `./internal/scheduler` | 1014 | 224 | Broad scheduler surface; quota resume advisory-lock, backpressure token cap, counter reconciler transaction, batch flusher, contract expiry, webhook message cleanup, watched query, and stats aggregator slices are clean with 123 killed mutants. Continue splitting by reapers, monitors, usage emailers, and reconciliation helpers before a full run. |
