...
| `./internal/queue` | 742 | 36 | Broad queue surface; enqueue terminal error helper, backpressure, enqueue retry, PgQue route selection, PgQue claim helper, PgQue dequeue, PgQue ready, PgQue enqueue, PgQue runtime, and health sampler slices are clean with 539 killed mutants. Continue splitting by notify, run writer, and metrics helpers before a full run. |
