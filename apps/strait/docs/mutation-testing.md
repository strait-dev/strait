...
| `./internal/api/grpc` | 380 | 146 | Broad worker-plane surface; auth, auth cache, replica ID, registry, recovery/logging interceptors, Sentry interceptor, worker metrics, server startup/TLS, dispatch helpers, sweep, DB sync, and stream control-loop helper slices are covered with 373 killed mutants and one intentional forced-stop timeout gap. Sweep has no lived mutants with 30 killed and 2 constant-only not covered after covering stale-task recovery, stale-worker eviction, offline deletion, durable result handoff, finalizer retry, and recovered-run requeue paths; DB sync has no lived mutants with 5 killed and 1 constant-only not covered after covering queue selection and register error continuation. Remaining dispatch/stream gaps are mostly DB-backed worker-task, audit, and end-to-end stream paths. |
