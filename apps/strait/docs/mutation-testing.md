...
| `./internal/queue` | 778 | 4 | Broad queue surface; enqueue terminal error helper, backpressure, enqueue retry, PgQue route selection, PgQue claim helper, PgQue dequeue, PgQue ready, PgQue enqueue, PgQue runtime, health sampler, notify, and run writer slices are clean with 633 killed mutants. Remaining uncovered mutants are PgQue priority constants and queue metrics callback guards. |
