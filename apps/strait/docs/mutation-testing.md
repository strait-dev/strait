...
| `./internal/store` | 424 | 2133 | Store package has many DB accessors with little direct self-coverage; API-key persistence has no lived mutants with 37 killed and 27 not covered after covering create, revoke, rotation guard, not-found mapping, and row scanning branches. Worker recovery should stay on smaller targeted integration slices: a full integration-tagged workers dry probe did not return quickly enough for the local mutation loop. |
