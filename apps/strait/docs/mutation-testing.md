...
| `./internal/scheduler` | 1040 | 198 | Broad scheduler surface; quota resume advisory-lock, backpressure token cap, counter reconciler transaction, cron, usage report and forecast emailers, batch flusher, contract expiry, webhook message cleanup, watched query, and stats aggregator slices are clean with 264 killed mutants. Continue splitting by reapers, monitors, usage flusher, and reconciliation helpers before a full run. |
