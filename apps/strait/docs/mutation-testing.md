...
| `./internal/billing` | 1105 | 411 | Large default-edition surface; retention resolver, entitlement snapshot, project cost aggregation, plan-limit resolution, catalog resolver, PostHog client, Prometheus uptime source, threshold warning, add-on limit, billing email sender, welcome email, webhook dispatcher, downgrade preview, SLA credit, and dunning slices are clean with bounded runs. Dunning has no lived mutants with 27 killed and 12 constant-only not covered after covering logger configuration plus deterministic initial and periodic run-loop errors. Split remaining work by enforcement, webhook, usage, store, and email subareas before a full real run. |
