...
| `./internal/scheduler` | 1027 | 211 | Broad scheduler surface; quota resume advisory-lock, backpressure token cap, counter reconciler transaction, cron, usage report emailer, batch flusher, contract expiry, webhook message cleanup, watched query, and stats aggregator slices are clean with 251 killed mutants. Continue splitting by reapers, monitors, usage emailers, and reconciliation helpers before a full run. |
