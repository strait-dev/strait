...
| `./internal/api/grpc` | 336 | 190 | Broad worker-plane surface; auth, auth cache, replica ID, registry, recovery/logging interceptors, Sentry interceptor, worker metrics, server startup/TLS, dispatch helpers, and stream helper slices are covered with 326 killed mutants and one intentional forced-stop timeout gap. Remaining dispatch/stream/sweep gaps are DB-backed worker-task, audit, and stream-loop paths. |
