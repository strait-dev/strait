...
| `./internal/scheduler` | 1041 | 197 | Broad scheduler surface; component registration, quota resume advisory-lock, backpressure token cap, counter reconciler transaction, cron, usage report and forecast emailers, usage flusher, batch flusher, contract expiry, webhook message cleanup, watched query, and stats aggregator slices are clean with 313 killed mutants. Continue splitting by reapers, monitors, and reconciliation helpers before a full run. |
