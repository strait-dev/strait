...
| `./internal/queue` | 715 | 62 | Broad queue surface; enqueue terminal error helper, backpressure, enqueue retry, PgQue route selection, PgQue claim helper, PgQue dequeue, PgQue ready, PgQue enqueue, and PgQue runtime slices are clean with 508 killed mutants. Continue splitting by health sampler, notify, run writer, and metrics helpers before a full run. |
