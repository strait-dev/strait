...
| `./internal/api/grpc` | 317 | 208 | Broad worker-plane surface; auth, auth cache, replica ID, registry, recovery/logging interceptors, Sentry interceptor, worker metrics, server startup/TLS, and dispatch helper slices are covered with 219 killed mutants and one intentional forced-stop timeout gap. Remaining dispatch gaps are DB-backed worker-task/audit paths; continue splitting by stream and sweep helpers before a full run. |
