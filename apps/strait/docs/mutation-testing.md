...
| `./internal/scheduler` | 1013 | 225 | Broad scheduler surface; quota resume advisory-lock, backpressure token cap, counter reconciler transaction, batch flusher, contract expiry, webhook message cleanup, and watched query slices are clean with 118 killed mutants. Continue splitting by reapers, monitors, usage emailers, and reconciliation helpers before a full run. |
