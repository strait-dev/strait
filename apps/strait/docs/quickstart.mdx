...
Follow these steps to set up strait locally and run your first job. You'll have a production-grade job strait running in under 10 minutes.
...
<Info>
The strait validates required environment variables on startup. Missing variables cause a clear error message with configuration instructions.
</Info>
...
Build the strait binary and start it in `all` mode (API + worker combined).
...
<CodeGroup>
  ...
  ```bash CLI
  # Using the strait CLI (recommended)
  ./strait jobs create \
    --name "Hello World" \
    --slug "hello-world" \
...
Trigger a run for the job you just created. The strait will enqueue the run and a worker will dispatch it to the endpoint.
...
<Info>
**Workflow Execution**: When triggered, the strait creates a `workflow_run`. Step `extract` executes first, then `transform` executes only after `extract` completes. The DAG engine handles fan-in automatically when multiple steps depend on the same parent.
</Info>
