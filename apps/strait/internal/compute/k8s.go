...
// validK8sNamespaceRe matches valid Kubernetes namespace names per RFC 1123 DNS label rules:
// lowercase letters, digits, and hyphens; must start and end with a letter or digit; max 63 chars.
var validK8sNamespaceRe = regexp.MustCompile(`^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$`)
...
// resolveNamespace returns the override namespace if set and valid, otherwise the default.
// Invalid namespace overrides are rejected (logged and ignored) to prevent tenant isolation bypass.
func (k *K8sRuntime) resolveNamespace(override string) string {
	if override != "" {
		if !validK8sNamespaceRe.MatchString(override) {
			slog.Warn("k8s: invalid namespace override ignored, using default",
				"override", override,
				"default", k.namespace,
			)
			return k.namespace
