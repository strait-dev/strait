...
func TestStripeUsageReporter_IngestRunUsage_EmptySecretNoop(t *testing.T) {
	t.Parallel()
	// Empty secret key causes IngestRunUsage to return nil without calling Stripe.
	reporter := NewStripeUsageReporter("", slog.Default())
	err := reporter.IngestRunUsage(context.Background(), "cust-123", "run-456")
	if err != nil {
...
func TestStripeUsageReporter_SkipsWhenNotConfigured(t *testing.T) {
	...
	err = reporter2.IngestRunUsage(context.Background(), "", "run-456")
	if err != nil {
