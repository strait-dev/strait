...
func TestAddonCap_UnderLimit_Allows(t *testing.T) {
	...
	mapping := NewPolarMappingFromOptions(
		WithStarterProducts("starter-id", ""),
		WithProProducts("pro-id", ""),
		WithAddonProduct("addon-conc", AddonConcurrentRuns),
	)
	...
	err := handler.handleAddonSubscriptionCreated(context.Background(), sub, AddonConcurrentRuns)
	if err != nil {
