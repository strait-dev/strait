...
func TestLoad_NotifyEmailProviderValidation(t *testing.T) {
	...
	_, err := Load()
	if err == nil {
		t.Fatal("expected error for invalid notify email provider")
	}
	if !strings.Contains(err.Error(), "NOTIFY_EMAIL_PROVIDER") {
		t.Fatalf("error = %q, want NOTIFY_EMAIL_PROVIDER", err.Error())
	}
}
