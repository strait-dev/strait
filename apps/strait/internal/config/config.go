...
type Config struct {
	...
	// Resend integration (billing email)
...
// CORS policy, notify subsystem, and audit subsystem invariants. It mutates
// cfg in two well-defined cases (community-edition ComputeRuntime override;
// no other assignments) to match the pre-refactor behavior of Load. Returns
// a *domain.ConfigError pinpointing the offending field, or nil on success.
//
...
func validateLoaded(cfg *Config) error {
	...
	case "", "ses":
		// valid
	default:
		return &domain.ConfigError{Field: "NOTIFY_EMAIL_PROVIDER", Message: "must be ses"}
	}
