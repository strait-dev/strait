...
func TestDrainController_RequestRequiresFullBatchHint(t *testing.T) {
	...
	select {
	case <-d.wakeChan():
		require.Fail(t, "partial batch should not request drain")
	default:
	}
	...
	case <-d.wakeChan():
	default:
		require.Fail(t, "full batch should request drain")
	}
...
func TestDrainController_CoalescesPendingWake(t *testing.T) {
	...
	case <-d.wakeChan():
	default:
		require.Fail(t, "first drain wake was not delivered")
	}

	select {
	case <-d.wakeChan():
		require.Fail(t, "second drain wake should be coalesced")
	default:
	}
}
