...
func (q *PgQueQueue) Enqueue(ctx context.Context, run *domain.JobRun) error {
	...
	if !ok {
		...
		if ready {
			if err := q.sendFreshReadyEvent(ctx, q.db, run); err != nil {
				return err
			}
		}
		return nil
...
func (q *PgQueQueue) EnqueueInTx(ctx context.Context, tx store.DBTX, run *domain.JobRun) error {
	...
	return q.sendFreshReadyEvent(ctx, tx, run)
}
...
func (q *PgQueQueue) EnqueueBatch(ctx context.Context, runs []*domain.JobRun) (int64, error) {
	...
	if !ok {
		...
		if err := q.sendFreshReadyEvents(ctx, q.db, runs); err != nil {
			return 0, err
		}
		return inserted, nil
	}
	...
	if err := q.sendFreshReadyEvents(ctx, tx, runs); err != nil {
		return 0, err
	}
	if err := tx.Commit(ctx); err != nil {
