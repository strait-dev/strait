...
func TestReaper_ReapExpiredApprovals(t *testing.T) {
	...
	t.Run("cost_gate_auto_approve_uses_callback_instead_of_timeout", func(t *testing.T) {
		t.Parallel()
		var timedOutApproval atomic.Int32
		var failedStep atomic.Int32
		...
		ms := &mockReaperStore{
			...
			getCostGateDefaultActionFn: func(_ context.Context, stepRunID string) (string, error) {
				if stepRunID != "sr-1" {
			...
			getWorkflowStepRunFn: func(_ context.Context, id string) (*domain.WorkflowStepRun, error) {
				if id != "sr-1" {
					t.Fatalf("step run id = %q, want sr-1", id)
				}
				return &domain.WorkflowStepRun{ID: "sr-1", StepRef: "expensive"}, nil
			},
			updateWorkflowApprovalFn: func(_ context.Context, _ string, _ string, _ string, _ *time.Time, _ string) error {
				timedOutApproval.Add(1)
				return nil
			},
		...
		cb := &mockWorkflowCallback{
			approveStepFn: func(_ context.Context, workflowRunID, stepRef, approver string) error {
				if workflowRunID != "wr-1" || stepRef != "expensive" || approver != "system:cost-gate-timeout" {
		...
		if approved.Load() != 1 {
			t.Fatalf("expected 1 auto-approval, got %d", approved.Load())
		}
		if timedOutApproval.Load() != 0 {
			t.Fatalf("expected 0 timed-out approval updates, got %d", timedOutApproval.Load())
		}
		if failedStep.Load() != 0 {
			t.Fatalf("expected 0 failed step updates, got %d", failedStep.Load())
		}
	})
