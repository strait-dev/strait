...
func TestSchedulerStop_ReturnsAfterComponentTimeout(t *testing.T) {
	...
	s.tracker.track(context.Background(), &s.wg, "stuck_component", func() {
		<-blocked
	})
...
func TestSchedulerStop_NoTimeoutWhenComponentsExitCleanly(t *testing.T) {
	...
	s.tracker.track(context.Background(), &s.wg, "clean_component", func() {
		<-release
	})
...
func TestSchedulerStop_ReportsTimedOutComponentCount(t *testing.T) {
	...
	blocked := make(chan struct{})
	s.tracker.track(context.Background(), &s.wg, "stuck_a", func() { <-blocked })
	s.tracker.track(context.Background(), &s.wg, "stuck_b", func() { <-blocked })
