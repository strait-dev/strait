...
- `packages/`: shared TS workspaces (`billing`, `config`, `transactional`) plus repo config and ops helpers (`configs`, `monitoring`, `scripts`). The `@strait/ui` component library is an external npm package, not a local workspace.
- `.github/workflows/`: CI
