...
impl EventTriggersService {
    ...
    pub async fn send_event(
        ...
        let path = substitute_path_params(
            "/v1/event-triggers/events/{eventKey}/send",
            &[("eventKey", event_key)],
        );
        self.client
    ...
    pub async fn send_prefix(
        &self,
        prefix: &str,
        body: serde_json::Value,
    ) -> Result<serde_json::Value, StraitError> {
        let path = substitute_path_params(
            "/v1/event-triggers/prefix/{prefix}/send",
            &[("prefix", prefix)],
        );
        self.client
            .do_request("POST", &path, None, None, Some(body))
    ...
    pub async fn purge_event(
        ...
        self.client
            .do_request("POST", "/v1/event-triggers/purge", None, None, Some(body))
            .await
    }

    pub async fn get_stat(&self) -> Result<serde_json::Value, StraitError> {
        self.client
            .do_request("GET", "/v1/event-triggers/stats", None, None, None)
            .await
    }
}
