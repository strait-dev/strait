...
impl EventSourcesService {
    ...
    pub async fn list(
        ...
        self.client
            .do_request("GET", "/v1/event-sources", query, None, None)
            .await
    }

    pub async fn create(&self, body: serde_json::Value) -> Result<serde_json::Value, StraitError> {
        self.client
            .do_request("POST", "/v1/event-sources", None, None, Some(body))
            .await
    }

    pub async fn get(&self, source_id: &str) -> Result<serde_json::Value, StraitError> {
        let path =
            substitute_path_params("/v1/event-sources/{sourceID}", &[("sourceID", source_id)]);
    ...
    pub async fn delete_subscription(
        ...
        let path = substitute_path_params(
            ...
            &[("sourceID", source_id), ("subID", sub_id)],
        );
        self.client
            .do_request_no_content("DELETE", &path, None, None, None)
            .await
    ...
    pub async fn dispatch_event(
        ...
        self.client
            .do_request("POST", "/v1/event-sources/dispatch", None, None, Some(body))
            .await
    }
}
