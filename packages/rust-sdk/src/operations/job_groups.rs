...
impl JobGroupsService {
    ...
    pub async fn list(&self, query: Option<&[(&str, &str)]>) -> Result<serde_json::Value, StraitError> {
        self.client.do_request("GET", "/v1/job-groups", query, None, None).await
    }

    pub async fn create(&self, body: serde_json::Value) -> Result<serde_json::Value, StraitError> {
        self.client.do_request("POST", "/v1/job-groups", None, None, Some(body)).await
    }
    ...
    pub async fn update(&self, group_id: &str, body: serde_json::Value) -> Result<serde_json::Value, StraitError> {
        let path = substitute_path_params("/v1/job-groups/{groupID}", &[("groupID", group_id)]);
    ...
    pub async fn list_jobs(&self, group_id: &str, query: Option<&[(&str, &str)]>) -> Result<serde_json::Value, StraitError> {
        let path = substitute_path_params("/v1/job-groups/{groupID}/jobs", &[("groupID", group_id)]);
    ...
    pub async fn pause_all(&self, group_id: &str) -> Result<serde_json::Value, StraitError> {
        let path = substitute_path_params("/v1/job-groups/{groupID}/pause", &[("groupID", group_id)]);
    ...
    pub async fn resume_all(&self, group_id: &str) -> Result<serde_json::Value, StraitError> {
        let path = substitute_path_params("/v1/job-groups/{groupID}/resume", &[("groupID", group_id)]);
        self.client.do_request("POST", &path, None, None, None).await
    }

    pub async fn get_stats(&self, group_id: &str) -> Result<serde_json::Value, StraitError> {
        let path = substitute_path_params("/v1/job-groups/{groupID}/stats", &[("groupID", group_id)]);
        self.client.do_request("GET", &path, None, None, None).await
    }
}
