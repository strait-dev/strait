#!/usr/bin/env bash
set -euo pipefail

# All SDKs have moved to dedicated repositories:
#   - TypeScript: https://github.com/strait-dev/strait-ts
#   - Python:     https://github.com/strait-dev/strait-python
#   - Go:         https://github.com/strait-dev/strait-go
#   - Ruby:       https://github.com/strait-dev/strait-ruby
#   - Rust:       https://github.com/strait-dev/strait-rust

echo "All SDKs have moved to dedicated repositories. Nothing to sync."
